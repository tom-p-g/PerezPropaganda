...
function renderizarProductos(catProductos, categoria) { //(lugar del HTML, categoría de los objetos)
    ...
    for (let i = 0; i < productosFiltrados.length; i++) {
        ...
        cartaProducto.innerHTML = `<div><img src="./img/prod/${String(productosFiltrados[i].id)}.jpg" alt="Foto de ${productosFiltrados[i].nombre}" class="imgProd"></div><ul class="listaProducto"><li class="idProd">${productosFiltrados[i].id}</li><li class="nomProd">${productosFiltrados[i].nombre}</li><li class="precioProd">Precio unitario: $ ${productosFiltrados[i].precioU}</li><li class="cantProd"><p>Ingrese cantidad que desea comprar</p><input name="cantidad" id="inputCant${String(productosFiltrados[i].id)}" class="inputCant"></li></ul><div class="botonesProd"><button type="button" class="btnCarrito" id="btn${productosFiltrados[i].id}">Añadir al Carrito</button><button type="button" class="infoProd">Mas Información</button></div>`
